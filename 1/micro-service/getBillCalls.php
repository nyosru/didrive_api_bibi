...
try {
    ...
    if (empty($_REQUEST['req_nom']) && empty($req_nom))
        throw new \Exception('нет номера запроса');
...
//        }
    ...
    if( isset($skip_end) && $skip_end === true ){
        return $r;
    }else{
    \f\end2('норм ' . $r, true, $_REQUEST);
    }
...
//  f\end2('ок');
    ...
    \f\end2($exc->detail->UssWsApiException->errorDescription, false);
